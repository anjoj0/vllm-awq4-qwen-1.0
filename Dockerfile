...
RUN apt-get update && apt-get install -y --no-install-recommends cargo rustc pkg-config && \
    rm -rf /var/lib/apt/lists/*
...
# torch/triton to our v2-staging wheels and keeps numpy/conch on the
# 2026-05-10 verified set instead of allowing runtime deps to downgrade them.
