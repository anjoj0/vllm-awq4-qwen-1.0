...
class RDNA3W4A16LinearKernel(MPLinearKernel):
    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, str | None]:
        ...
        if c.weight_type == scalar_types.uint4 and not c.zero_points:
            return (
                False,
                "Asymmetric uint4 requires explicit zero points for the "
                "RDNA3 W4A16 kernel",
            )
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        def transform_w_zp(x):
            groups = c.partition_weight_shape[0] // c.group_size
            packed_out = c.partition_weight_shape[1] // 8
            expected = (groups, packed_out)
            checkpoint_layout = (packed_out, groups)
            shape = tuple(x.data.shape)

            if shape == expected:
                x.data = x.data.contiguous()
            elif shape == checkpoint_layout:
                # compressed-tensors stores [N/8, K/G], while the HIP op
                # consumes the ExLlama/GPTQ layout [K/G, N/8].
                x.data = x.data.t().contiguous()
            else:
                raise ValueError(
                    "Unexpected RDNA3 W4A16 zero-point shape "
                    f"{shape}; expected {expected} or {checkpoint_layout}"
                )
            return x
    ...
    def apply_weights(
        ...
        # Explicit zero points from compressed-tensors store the effective
        # value directly. Synthesized GPTQv1 zeros retain the historical +1
        # offset expected by ExLlama-compatible checkpoints.
        use_v2_format = c.weight_type == scalar_types.uint4 and c.zero_points
