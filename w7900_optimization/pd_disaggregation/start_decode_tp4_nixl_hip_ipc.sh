...
export LD_LIBRARY_PATH="${PD_ROOT}/venv/lib/x86_64-linux-gnu:${LD_LIBRARY_PATH:-}"
...
export VLLM_PORT="${VLLM_PORT:-30001}"
...
exec python -m vllm.entrypoints.cli.main serve "${MODEL_PATH}" \
  ...
  --enforce-eager \
  --kv-transfer-config '{"kv_connector":"NixlConnector","kv_role":"kv_consumer","kv_load_failure_policy":"fail","kv_buffer_device":"cuda","kv_connector_extra_config":{"backends":["W7900_HIP_IPC"]}}'
