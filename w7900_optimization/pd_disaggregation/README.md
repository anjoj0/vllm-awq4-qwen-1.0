...
该目录在 8 张 Radeon PRO W7900 上实现 Qwen3.6-27B BF16 的 TP=4 Prefill 与 TP=4 Decode 解耦。传输使用 vLLM 原生 `NixlConnector`、NIXL 1.4.0 和带 ROCm memory domain 的 UCX 1.22，不依赖 LMCache。
...
- 当前 W7900/Navi31 节点可完成 Attention KV、Mamba conv state 和 SSM state 迁移，但 UCX 大块 RMA 数据仍回退到 `tcp/bond0`，没有使用 `rocm_ipc` 数据通道。
...
因此该实验补丁不进入默认环境。`UCX_TLS` 仍需包含 `tcp`，因为 `rocm_ipc`/`rocm_copy` 不提供 NIXL 所需的 active-message 控制通道。
