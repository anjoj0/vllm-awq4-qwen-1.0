...
nixl_status_t W7900HipIpcBackend::registerMem(const nixlBlobDesc& mem,
    ...
    size_t allocation_size = 0;
    hipError_t status = hipMemGetAddressRange(
...
nixl_status_t W7900HipIpcBackend::loadRemoteMD(const nixlBlobDesc& input,
    ...
    std::shared_ptr<RemoteMapping> mapping;
    {
        ...
        if (!mapping) {
            ...
            void* mapped = nullptr;
            const hipError_t status = hipIpcOpenMemHandle(
...
nixl_status_t W7900HipIpcBackend::prepXfer(const nixl_xfer_op_t& operation,
    ...
    for (size_t i = 0; i < local.descCount(); ++i) {
        ...
        if (remote[i].addr < mapping->remote_base ||
            remote[i].addr + remote[i].len >
                mapping->remote_base + mapping->allocation_size)
            return NIXL_ERR_INVALID_PARAM;
...
nixl_status_t W7900HipIpcBackend::postXfer(const nixl_xfer_op_t&,
    ...
    if (request == nullptr || request->submitted) return NIXL_ERR_INVALID_PARAM;
    request->remote_agent = remote_agent;
    if (opt_args != nullptr && opt_args->hasNotif)
        request->notification = opt_args->notifMsg;
    ...
    if (status == hipSuccess) status = hipEventRecord(request->start, stream_);
    ...
    if (status == hipSuccess) status = hipEventRecord(request->end, stream_);
    if (status != hipSuccess) return hipStatus(status, "HIP IPC transfer submission");
...
nixl_status_t W7900HipIpcBackend::checkXfer(nixlBackendReqH* handle) const {
    auto* request = static_cast<TransferHandle*>(handle);
    if (request == nullptr || !request->submitted) return NIXL_ERR_INVALID_PARAM;
    const hipError_t status = hipEventQuery(request->end);
    if (status == hipErrorNotReady) return NIXL_IN_PROG;
    if (status != hipSuccess) return hipStatus(status, "hipEventQuery");
    if (!request->notification.empty() && !request->notification_sent) {
        ...
        if (notif_status != NIXL_SUCCESS) return notif_status;
...
nixl_status_t W7900HipIpcBackend::releaseReqH(nixlBackendReqH* handle) const {
    ...
    (void)hipSetDevice(device_);
    if (request->end != nullptr) {
        (void)hipEventSynchronize(request->end);
        (void)hipEventDestroy(request->end);
    }
...
nixl_status_t W7900HipIpcBackend::sendNotif(const std::string& remote_agent,
    ...
    return sent == static_cast<ssize_t>(payload.size()) ? NIXL_SUCCESS : NIXL_ERR_BACKEND;
}
...
nixl_status_t W7900HipIpcBackend::getNotifs(notif_list_t& notif_list) {
    ...
    while (true) {
        const ssize_t bytes = recv(notif_fd_, buffer.data(), buffer.size(), 0);
        if (bytes < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) break;
        if (bytes < 0) return NIXL_ERR_BACKEND;
        ...
        const size_t expected = 2 * sizeof(uint32_t) + sender_size + message_size;
