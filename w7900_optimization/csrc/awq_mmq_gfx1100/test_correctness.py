"""
Correctness test for AWQ-INT4 MMQ HIP custom op (gfx1100).
...
def run_one(M, N, K, version, group_size=32, atol=1e-2, x_scale=0.1):
    ...
    if version == 9:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v9(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 8:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v8(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 7:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v7(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 6:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v6(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 5:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v5(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 4:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    elif version == 3:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm_kmajor(
            x, pack_kmajor_weights(w_packed), scales.t().contiguous())
    else:
        out_ours = awq_mmq_gfx1100.mmq_q4_gemm(x, w_packed, scales, version=version)
