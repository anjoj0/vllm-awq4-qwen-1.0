...
def mmq_q4_gemm(
    ...
    """
    AWQ-INT4 MMQ kernel for gfx1100.
    ...
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm(x, w_packed, scales, w_zeros, version)
...
def mmq_q4_gemm_kmajor(
    ...
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor(
...
def mmq_q4_gemm_kmajor_wmma(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma(
        x, w_packed_t, scales_t, zeros_t)
...
def mmq_q4_gemm_kmajor_wmma_v5(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v5(
        x, w_packed_t, scales_t, zeros_t)
...
def mmq_q4_gemm_kmajor_wmma_v6(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v6(
        x, w_packed_t, scales_t, zeros_t)
...
def mmq_q4_gemm_kmajor_wmma_v7(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v7(
        x, w_packed_t, scales_t, zeros_t)
...
def mmq_q4_gemm_kmajor_wmma_v8(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v8(
        x, w_packed_t, scales_t, zeros_t)
...
def mmq_q4_gemm_kmajor_wmma_v9(
    ...
    if zeros_t is None:
        zeros_t = torch.empty(0, dtype=torch.int32, device=x.device)
    return torch.ops.awq_mmq_gfx1100.mmq_q4_gemm_kmajor_wmma_v9(
        x, w_packed_t, scales_t, zeros_t)
